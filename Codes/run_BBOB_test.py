# Function group: 1,2,3,[4~17] are variations of 1,2, 18, [19,20,21], 22, 23, 24, 25
import argparse
import json
import os.path
from pathlib import Path

from ioh import get_problem

from ThesisCode.KrigingSMT import createBBOBKrgConfig
from ThesisCode.Pipeline import pipeline
from ThesisCode.RBFRoy import createBBOBRBFkernels
from ThesisCode.SVMSklearn import defaultSVMsettings
from ThesisCode.utils import createIntInputSpace, IOH_int_function_new


def create_func(f_id, iid, func_dim):
    func = get_problem(f_id, iid, func_dim, 'PBO')
    return func


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run experiments according to all_config.json')
    parser.add_argument('-F', metavar='Functions', type=int,
                        help='1~6, the index of function groups', required=True)
    parser.add_argument('-I', metavar='Instance', type=int,
                        help='1~2, the index of instance', default=1)
    parser.add_argument('-D', metavar='Dimensionality', type=int, default=2,
                        help="1~3, dimensionality of the problem, 25 or 36.")
    parser.add_argument('-A', metavar='Algorithms', type=int,
                        help='1~12, benchmark functions, 9(PV-ES), 10(EI-ES), 11(PV-EA) and 12(EI-EA) is our algorithms.',
                        required=True)
    # parser.add_argument('-W', metavar='Weights', type=int, nargs=1, default=1, help='1, for half-half')
    parser.add_argument('-R', metavar='Run', type=int, required=True, help='the index of an independent run')
    args = parser.parse_args()
    input_configs = vars(args)
    # Load configurations from BBOB_config.json
    try:
        with open('./BBOB_config.json') as config_file:
            config = json.load(config_file)
    except FileNotFoundError:
        raise FileNotFoundError
    Fid = input_configs['F'] - 1
    Did = input_configs['D'] - 1
    Iid = input_configs['I'] - 1
    Aid = input_configs['A'] - 1
    func_id = config['Func_ids'][Fid]
    # Dimensionality of problems
    dims = config['Dims'][Did]
    # Not useful for BBOB
    iid = config['Inst'][Iid]
    if Aid < 8:
        Alg_names = config['Algorithms'][Aid]
    else:
        Alg_names = 'Ours'
    # Ordinal problems
    Space = 'O'
    # Choices per variable
    nmax = 100
    int_mask = [nmax] * dims
    func = IOH_int_function_new(func_id, dims, iid, int_mask)
    func1 = IOH_int_function_new(func_id, dims, iid, int_mask)
    runNo = input_configs['R']
    # Log Problem ID
    caseName = './Case' + str(Fid + 1) + '/'

    logName = 'run_num_' + str(runNo) + '.csv'
    newPath = os.path.join(caseName, Alg_names)
    Path(newPath).mkdir(parents=True, exist_ok=True)
    logs = os.path.join(newPath, logName)

    # Print function Name
    print(func1.f)
    if Aid == 8: # SAMA-DiEGO-A
        dtype, drange = createIntInputSpace(func)
        fp = 'case1'
        fp = os.path.join(newPath, fp)
        maxIter = 500
        Krg = createBBOBKrgConfig()
        RBF = createBBOBRBFkernels()
        svm = defaultSVMsettings(dims)
        foundX, foundY = pipeline(func, drange, dtype, maxEval=maxIter, minimization=True,
                                  KrgType=Krg, RBFKernels=RBF, SVMConfig=svm,
                                  runNo=runNo, nSingleModel=4, nTotal=7,
                                  optimizer_log=None, filePath=fp, ICtype='PV', Optimizer='mies', KrgGap=1,
                                  Space=Space)
    elif Aid == 9: # SAMA-DiEGO-B
        dtype, drange = createIntInputSpace(func)
        fp = 'case2'
        fp = os.path.join(newPath, fp)
        maxIter = 500
        Krg = createBBOBKrgConfig()
        RBF = createBBOBRBFkernels()
        svm = defaultSVMsettings(dims)
        foundX, foundY = pipeline(func, drange, dtype, maxEval=maxIter, minimization=True,
                                  KrgType=Krg, RBFKernels=RBF, SVMConfig=svm,
                                  runNo=runNo, nSingleModel=4, nTotal=7,
                                  optimizer_log=None, ICtype='EI', filePath=fp, Optimizer='mies', KrgGap=1,
                                  Space=Space)
